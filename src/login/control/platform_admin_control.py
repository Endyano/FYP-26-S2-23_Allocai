...
class PlatformAdminControl:
    ...
    def delete_company(company_id, user_id):
        company = CompanyEntity.update_status(company_id, "Deleted")
    ...
    def moderate_review(review_id, review_status):
        if review_status not in ["Approved", "Rejected", "Hidden"]:
            return {
                "success": False,
                "message": "Review status must be Approved, Rejected, or Hidden."
            }

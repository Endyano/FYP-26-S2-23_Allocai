...
export default function UsersPage() {
  ...
  async function loadStaff(search?: string, skillsetId?: string) {
    setLoading(true);
    setError('');
    const params = new URLSearchParams();
    if (search) params.set('search', search);
    if (skillsetId) params.set('skillset_id', skillsetId);
  ...
  useEffect(() => {
    const handle = setTimeout(() => loadStaff(searchQuery, skillsetFilter), 300);
    return () => clearTimeout(handle);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchQuery, skillsetFilter]);
  ...
  async function submitHourLimit() {
    ...
    try {
      ...
      if (result.success) {
        ...
        await loadStaff(searchQuery, skillsetFilter);
  ...
  return (
    <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3 shadow-sm border border-slate-200 flex-1">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-slate-400 shrink-0"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          <input
            type="text"
        ...
        <div className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3 shadow-sm border border-slate-200 sm:w-64">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-slate-400 shrink-0"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></svg>
          <select
            value={skillsetFilter}
            onChange={e => setSkillsetFilter(e.target.value)}
            className="w-full bg-transparent text-sm text-slate-900 font-medium outline-none"
          >
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center text-slate-500">
                    {searchQuery || skillsetFilter ? 'No staff found matching these filters.' : 'No staff members yet.'}
                  </td>
                </tr>
              )}
            </tbody>
